...
filepath_in = f'/fp/projects01/ec332/data/latent_spaces/file_{window}_{threshold}_sam2.npz'
bin_specs_arr = np.load(filepath_in)['latent_spaces']
...
# Save Model
model_save_path = f'models/file_{window}_{threshold}_model_{num_epochs}_{pos_weight}_{learning_rate}_{batch_size}_{bottle_filters}_sam2.pth'
...
np.savez(f"model_loss/file_{window}_{threshold}_model_{num_epochs}_{pos_weight}_{learning_rate}_{batch_size}_{bottle_filters}_sam2.npz", train_loss=np.array(train_loss_list), val_loss= np.array(val_loss_list))
