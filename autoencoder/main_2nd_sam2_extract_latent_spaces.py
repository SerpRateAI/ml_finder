...
foundational_array = np.load(f'/fp/projects01/ec332/data/latent_spaces/file_{window}_{threshold}_sam2.npz')['latent_spaces']
data = torch.tensor(foundational_array, dtype=torch.float32)
...
class ConvAutoencoder(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Conv2d(16, 16, kernel_size=3, padding=1), 
            nn.ReLU(True),
            nn.ConvTranspose2d(16, 256, kernel_size=3, stride=2, padding=1, output_padding=1),
            nn.Sigmoid()
        )
    def forward(self, x):
...
model_path = f'models/file_{window}_{threshold}_model_{num_epochs}_{weight}_{learning_rate}_{batch_size}_{bottle}_sam2.pth'
...
np.savez(f"/fp/projects01/ec332/data/latent_spaces/file_{window}_{threshold}_model_{num_epochs}_{weight}_{learning_rate}_{batch_size}_{bottle}_2nd_sam.npz", all_latent_spaces=all_latent_spaces)
